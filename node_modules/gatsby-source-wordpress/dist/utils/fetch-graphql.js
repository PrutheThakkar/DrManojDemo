...
const handleGraphQLErrors = async ({
  ...
  for (const error of errors) {
    var _error$path, _error$path$map, _errorPath, _error$extensions$deb, _error$extensions;
    ...
    const debugMessage = (_error$extensions$deb = (_error$extensions = error.extensions) === null || _error$extensions === void 0 ? void 0 : _error$extensions.debugMessage) !== null && _error$extensions$deb !== void 0 ? _error$extensions$deb : error.debugMessage;
    if (debugMessage) {
      reporter.error((0, _formatLogMessage.formatLogMessage)(`${error.category ? `Error category: ${error.category} \n\n` : ``}Error: \n  ${error.message} \n\n Debug message: \n  ${debugMessage} \n\n Error path: ${errorPath}`));
    } else {
      var _error$locations, _error$locations$map;
      reporter.error((0, _formatLogMessage.formatLogMessage)(`${error.category ? `(${error.category}) ` : ``}${error !== null && error !== void 0 && (_error$locations = error.locations) !== null && _error$locations !== void 0 && _error$locations.length ? (_error$locations$map = error.locations.map(location => `location: line ${location.line}, column: ${location.column}`)) === null || _error$locations$map === void 0 ? void 0 : _error$locations$map.join(`. `) : ``} \n\t ${error.message}  \n\n Error path: ${errorPath} \n\n If you haven't already, try adding ${(0, _chalk.bold)(`define( 'GRAPHQL_DEBUG', true );`)} to your wp-config.php for more detailed error messages.`));
    }
  }
  await handleErrors({
    responseJSON: json,
